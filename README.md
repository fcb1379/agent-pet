...
上位机支持把录音模块产出的 Ogg Opus 等音频分片周期性提交给内置 STT，并在桌宠中实时显示为可编辑的“对话输入”。内置 WASM 解码器会先把 Ogg Opus 转为 16 kHz WAV；whisper.cpp 运行时和中文模型随 Windows 发布包预置。应用启动后会自动隐藏拉起服务、等待就绪并在异常退出后恢复，用户不需要安装、启动或配置 STT/FFmpeg。

服务只绑定随机分配的本机回环端口，音频不会发送到公网。可从托盘菜单“本地语音转写”查看加载状态、选择音频文件测试或清空转写稿。录音模块的分片接入接口与格式约束见 [上位机本地实时语音转写](docs/11-本地实时语音转写.md)。当前转写稿不会自动代替用户向 Codex 或 Claude Code 提交文本。
